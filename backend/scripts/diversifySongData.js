  ...
  `;
    ...
    for (const row of rows) {
        ...
        for (let i = 0; i < additional; i++) {
            const duration = Math.min(songDuration, Math.max(MIN_RANDOM_DURATION, randomInt(MIN_RANDOM_DURATION, songDuration)));
...
async function adjustLikes() {
    const pool = await getPool();
    const [likeCountsRows] = await pool.query(`
...
main();
//# sourceMappingURL=diversifySongData.js.map