 ...
 * when they drop below 20 followers.
 * 
...
const addUnverifyArtistTrigger = async () => {
  try {
    ...
    const triggerExists = db.prepare(`
      ...
      WHERE type='trigger' AND name='unverify_artist_below_20_followers'
    `).get();
    ...
    // Create the trigger
    console.log('Creating trigger: unverify_artist_below_20_followers');
    ...
    console.log('\nChecking for artists with < 20 followers that are still verified...');
    ...
    console.log('  - Unverify artists when they drop below 20 followers');
